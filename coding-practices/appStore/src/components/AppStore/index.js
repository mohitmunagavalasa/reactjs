...
class AppStore extends Component {
  ...
  getFilteredApps = () => {
    ...
    const filteredApps = appsList.filter(
      appDetails => activeTabId === appDetails.category,
    )
    return filteredApps
  }
  ...
  onChangeInputText = event => {
    this.setState({
      activeAppName: event.target.value,
    })
  }

  render() {
    const {activeTabId, activeAppName} = this.state
    const filteredApps = this.getFilteredApps()
    const searchFilteredApps = filteredApps.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(activeAppName.toLowerCase()),
    )
      ...
      <div className="app-store-bg-container">
        <div className="bg-container">
          ...
          <ul className="app-items-container">
            {searchFilteredApps.map(appDetails => (
